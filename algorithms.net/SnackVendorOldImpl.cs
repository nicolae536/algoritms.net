        ...
        public IEnumerable<TNode> FindPath()
        {
            while (notEvaluatedNodes.Count > 0)
            {
                ...
                foreach (TNode neighboar in heuristicImpl.GetNeighboars(current))
                {
                    ...
                    double tentativeDistance = costFromStartToNode[current] + heuristicImpl.DistanceBetween(current, neighboar);
                    // This is not a better path.
                    double costOrDefault = costFromStartToNode.ContainsKey(neighboar) ? costFromStartToNode[neighboar] : double.MaxValue;
                    ...
                    notEvaluatedNodes = notEvaluatedNodes.OrderBy(keyIt => costToGoal[keyIt.Key]).ToDictionary((keyItem) => keyItem.Key, (valueItem) => valueItem.Value);
                }
            }
