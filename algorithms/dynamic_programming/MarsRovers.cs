...
namespace algorithms.dynamic_programming
{
    class MarsRover
    {
        ...
        private void DriveInCircle(double distanceToDrive)
        {
            ...
            double tanY = yStart + 1;
            double tanX = (
                            (Math.Pow(tournRadius, 2) - ((yStart - yCenter) * (tanY - yCenter))) /
                            (xStart - xCenter)
                            )
                            + xCenter;
            ...
            double sinOfA = GetNumber(Math.Asin(normalToTanPoint / pointToTanLength));
            newDirection = GetDegrees(sinOfA);
        }
